...
WITH agg AS (
    SELECT 
        ...
        OBJECT_CONSTRUCT(
            ...
            'THR_SSN_NULL', COUNT_IF(SSN IS NULL),
            'THR_RECORD_DROP_PCT', DIV0(
                COUNT_IF(BUSINESS_DATE=CURRENT_DATE-2)-COUNT_IF(BUSINESS_DATE=CURRENT_DATE-1),
                COUNT_IF(BUSINESS_DATE=CURRENT_DATE-2)
            ) * 100
        ) AS METRICS_OBJ
...
WITH agg AS (
    SELECT 
        ...
        OBJECT_CONSTRUCT(
            ...
            'VAL_EMAIL_FORMAT', COUNT_IF(IS_ACTIVE=TRUE 
                AND PRIMARY_EMAIL_VALID_FLAG=1
                AND PRIMARY_EMAIL_ADDRESS IS NOT NULL 
                AND PRIMARY_EMAIL_ADDRESS NOT LIKE '%@%'),
            'VAL_PHONE_FORMAT', COUNT_IF(IS_ACTIVE=TRUE 
                AND PRIMARY_PHONE_VALID_FLAG=1
                AND PRIMARY_PHONE_NUMBER IS NOT NULL 
